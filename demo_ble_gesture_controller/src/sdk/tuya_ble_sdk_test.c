    ...
    switch (cmd->sub_id)
    {
        ...
        case TY_UARTV_CMD_SET_PID: {
            if(tuya_ble_device_update_product_id(cmd->value[0], cmd->value[1], &cmd->value[2]) == TUYA_BLE_SUCCESS) {
                ...
                extern uint32_t ty_system_set_pid(uint8_t* buf, uint32_t size);
                ty_system_set_pid(&cmd->value[0], test_param.pid.len+2);
#endif
...
#if CHIP_PLATFORM_NRF52832
            extern uint32_t ty_system_get_pid(uint8_t* buf, uint32_t* p_size);
            ty_system_get_pid((void*)test_param.pid.value, (void*)&test_param.pid.len);
#endif
        ...
        case TY_UARTV_CMD_DP_REPORT: {
            if(cmd->value[0] == 0) {
                rsp_data[rsp_len] = tuya_ble_dp_data_send(g_sn++, cmd->value[1], cmd->value[2], cmd->value[3], &cmd->value[4], data_len-4);
            } else if(cmd->value[0] == 1) {
                if(cmd->value[4] == 0) {
                    rsp_data[rsp_len] = tuya_ble_dp_data_with_time_send(g_sn++, cmd->value[2], cmd->value[4], &cmd->value[5], &cmd->value[18], data_len-18);
                } else {
                    rsp_data[rsp_len] = tuya_ble_dp_data_with_time_send(g_sn++, cmd->value[2], cmd->value[4], &cmd->value[5], &cmd->value[9], data_len-9);
                }
                ...
                if(tmp_buf) {
                    ...
                    rsp_data[rsp_len] = tuya_ble_dp_data_send(g_sn++, cmd->value[1], cmd->value[2], cmd->value[3], tmp_buf, dp_data_len+4);
        ...
        case TY_UARTV_CMD_DP_REPORT_PERIOD: {
            ...
            if(buf[0] == 0) {
                tuya_ble_dp_data_send(g_sn++, buf[1], buf[2], buf[3], &buf[4], len-4);
            } else {
                if(buf[4] == 0) {
                    tuya_ble_dp_data_with_time_send(g_sn++, buf[2], buf[4], &buf[5], &buf[18], len-18);
                } else {
                    tuya_ble_dp_data_with_time_send(g_sn++, buf[2], buf[4], &buf[5], &buf[9], len-9);
                }
            }
